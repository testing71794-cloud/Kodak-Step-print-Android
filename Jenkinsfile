...
pipeline {
    ...
    stages {
        ...
        stage('Archive Reports & Artifacts') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    archiveArtifacts artifacts: 'build-summary/final_execution_report.xlsx, build-summary/failed_tests_artifacts.zip, build-summary/failed_tests_summary.json, build-summary/execution_logs.zip, .maestro/screenshots/**, detected_devices.txt', allowEmptyArchive: true
                }
            }
        }
