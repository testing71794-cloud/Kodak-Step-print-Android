...
def anyAtpModuleEnabled = {
    return params.RUN_ATP_ONBOARDING || params.RUN_ATP_SIGNUP_LOGIN ||
        params.RUN_ATP_CAMERA || params.RUN_ATP_COLLAGE ||
...
def atpSuiteIdsList = {
    return [
        'atp_onboarding', 'atp_signup_login',
...
/** GraalJS host access for in-flow adb capture (GA_02 OpenRouter verify). */
def galleryOpenRouterEnvList() {
    return [
...
/** Appium W3C pinch for GA_05 / GA_06 (real multitouch on gallery photo detail). */
...
pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'RUN_ATP_ONBOARDING', defaultValue: false, description: 'ATP TestCase Flows/onboarding')
        booleanParam(name: 'RUN_ATP_SIGNUP_LOGIN', defaultValue: false, description: 'ATP TestCase Flows/signup-login')
        ...
        booleanParam(name: 'RUN_ATP_EDITING', defaultValue: false, description: 'ATP TestCase Flows/editing')
        ...
        booleanParam(name: 'RUN_AI_ANALYSIS', defaultValue: false, description: 'Post-run OpenRouter failure analysis (intelligent_platform)')
        booleanParam(name: 'SEND_FINAL_EMAIL', defaultValue: false, description: 'Send final summary email')
        booleanParam(name: 'CLEAR_STATE', defaultValue: false, description: 'Clear app state in suite runners (adb pm clear + Maestro runner)')
        ...
        string(
            name: 'ATP_FLOW_INCLUDE',
            defaultValue: '',
            description: 'Optional: run only flows whose filename contains this text (e.g. GA_02). Applies within each enabled ATP module. Leave empty to run all flows in that module.'
        )
        string(
            name: 'OPENROUTER_MODEL_VISION',
            defaultValue: 'openai/gpt-4.1-mini',
            description: 'OpenRouter vision model for in-flow AI verify (GA_02 3x3 grid, ED_03, etc.).'
        )
    }
    ...
    stages {
        ...
        stage('Detect Connected Devices') {
            ...
            steps {
                catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                    script {
                        echo "[pipeline] ATP stages: Onboarding, SignUp_Login, Camera, Collage, Gallery, PreCut, Editing, Settings | CLEAR_STATE=${params.CLEAR_STATE}"
                        withEnv(maestroEnvList()) {
                ...
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    script {
                        withOpenRouterCredentials(params.OPENROUTER_CREDENTIALS_ID) {
                            withEnv(maestroEnvList() + galleryOpenRouterEnvList() + galleryAppiumPinchEnvList() + [
                                "ATP_FLOW_INCLUDE=${params.ATP_FLOW_INCLUDE}",
                                "OPENROUTER_MODEL_VISION=${params.OPENROUTER_MODEL_VISION}",
                                "OpenRouterAPI=${env.OPENROUTER_API_KEY ?: ''}",
        ...
        stage('Collect failed test artifacts') {
            ...
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    bat """call scripts\\jenkins_ci_collect_failed_artifacts.bat "${env.WORKSPACE}" """
                }
            }
        }
