#!/usr/bin/env python3
"""OpenRouter vision verify for GA_05 gallery pinch — zoom must change visibly."""
...
_PROMPT = """You analyze THREE Kodak Step Prints gallery photo DETAIL screenshots (same photo, white frame, Edit/Print/Collage bar).

Image A: BEFORE any Appium pinch (default fit).
Image B: AFTER pinch-out / spread (zoom IN setup — photo should look larger / more cropped in frame).
Image C: AFTER pinch-in (zoom OUT — photo should look smaller / more margin than B, closer to A).
...
- zoom_out_applied=true ONLY if C clearly differs from B with a zoomed-out / smaller photo (more black margin or less crop).
- still_on_detail_screen=true if C still shows photo detail with Edit/Print/Collage, NOT gallery grid.
- If B looks identical to A, zoom_in_applied=false. If C looks identical to B, zoom_out_applied=false."""
...
def verify_three(before: Path, zoomed: Path, after: Path) -> dict:
    ...
    messages = [
        {"role": "system", "content": _PROMPT},
        {
            ...
            "content": [
                ...
                _encode(before),
                {"type": "text", "text": "Image B (after pinch-out / zoomed in):"},
                _encode(zoomed),
                {"type": "text", "text": "Image C (after pinch-in / zoom out):"},
                _encode(after),
            ],
        },
    ]
...
def main() -> int:
    ...
    parser.add_argument("--zoomed", type=Path, help="after_pinch_out screenshot")
    parser.add_argument("--after", type=Path, help="after_pinch screenshot")
    args = parser.parse_args()

    w3c = _REPO / "automation" / "appium-gestures" / "target" / "screenshots" / "w3c"
    before = args.before or _latest_w3c("before_pinch", w3c)
    zoomed = args.zoomed or _latest_w3c("after_pinch_out", w3c)
    after = args.after or _latest_w3c("after_pinch", w3c)

    missing = [
        name
        for name, path in [("before", before), ("zoomed", zoomed), ("after", after)]
